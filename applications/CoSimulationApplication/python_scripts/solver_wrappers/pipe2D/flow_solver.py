...
class SolverWrapperPipeFlow(CoSimulationComponent):
    ...
    def __init__(self, parameters):
        ...
        start = self.settings["start"].GetDouble()  # Starting point
        ...
        self.disp = np.zeros((self.m + 2,3))
        self.traction = np.zeros((self.m + 2, 3))
        ...
        self.model_part.AddNodalSolutionStepVariable(self.variable_disp) #Don't work with area anymore
        ...
        for i in range(len(self.z)):
            self.model_part.CreateNewNode(i, 0.0, self.d/2.0, self.z[i]) #Use nodal y-coordinate as radial coordinate
        step = 0
        for node in self.model_part.Nodes:
            node.SetSolutionStepValue(self.variable_disp, step, self.disp[0,:].tolist()) # Initial displacement is 0 for all
        ...
        # print(self.interface_input.GetPythonList())
    ...
    def SolveSolutionStep(self, interface_input):
        ...
        # Changed input to a displacement
        self.interface_input.SetPythonList(interface_input.GetPythonList())
        disp = interface_input.GetPythonList()

        #update area
        self.a[1:self.m + 1] = m.pi * (self.d + np.array(disp[1::3]) * 2) ** 2 / 4.0  # Area of cross section
        ...
        tr = self.traction[1:self.m + 1,:]
        index_p = 0
        index_tr = 0
        for node in self.model_part.Nodes:
            node.SetSolutionStepValue(vars(KM)["PRESSURE"], 0, p[index_p])
            node.SetSolutionStepValue(vars(KM)["TRACTION"], 0, tr[index_tr,:])
            index_p += 1
            index_tr += 1
        # self.interface_output.SetNumpyArray(p*self.rhof)
        # print("END_SOLUTION_STEP_FLOW")
