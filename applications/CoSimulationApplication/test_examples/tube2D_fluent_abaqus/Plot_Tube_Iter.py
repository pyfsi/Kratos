...
dir_Tango = '/cfdfile2/data/fm/lucas/Tango_Test/Data/'
dir_CoCo = './'
...
file = dir_CoCo + "CSM/" + f"CSM_Time0Surface0Nodes.dat"
pos0_CoCo = np.array(pd.read_csv(file, header=None, skiprows=1, sep="\s+"))

for t in T:
    for i in range(12,it_max):
        ...
        file_CF = dir_CoCo + "CFD/" + f"nodes_update_timestep{t}_thread3_Iter{i}.dat"
        file_CA = dir_CoCo + "CSM/" + f"CSM_Time{t}Surface0Output_Iter{i}.dat"
        b=0
        b2 = 0
        ...
        if os.path.exists(file_TF):
            ...
            pos = pos[sort_id,:]
            p = plt.plot(pos[:,0],pos[:,1],label=f"Tango_Fluent_In_Iter{i}")
            b=1
        ...
        if os.path.exists(file_CF):
            ...
            pos = pos[sort_id, :]
            if b ==1:
                plt.plot(pos[:, 0], pos[:, 1],linestyle='--',label=f"CoCoNuT_Fluent_In_Iter{i}",color=p[0].get_color())
            else:
                plt.plot(pos[:, 0], pos[:, 1], linestyle='--', label=f"CoCoNuT_Fluent_In_Iter{i}")
        ...
        if os.path.exists(file_TA):
            ...
            sort_id = np.argsort(pos[:, 0])
            pos = pos[sort_id,:]
            p = plt.plot(pos[:,0],pos[:,1],label=f"Tango_Abaq_Out_Iter{i}")
            b2 = 1
        ...
        if os.path.exists(file_CA):
            ...
            pos = pos[sort_id, :]
            if b2 == 1:
                plt.plot(pos[:, 0], pos[:, 1],linestyle='--',label=f"CoCoNuT_Abaq_Out_Iter{i}",color=p[0].get_color())
            else:
                plt.plot(pos[:, 0], pos[:, 1], linestyle='--', label=f"CoCoNuT_Abaq_Out_Iter{i}")
        plt.figure(4*t+1)
        leg = plt.legend()
        leg.draggable()
        plt.figure(4*t+2)
